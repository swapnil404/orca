...
export function ProvisionPanel({ type, clusters, snapshot, busy, error, onCancel, onClose, onConfirm }: ProvisionPanelProps) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (type === 'pgbouncer') onConfirm({ type, clusterID: selectedClusterID, poolMode: String(form.get('pool_mode')) as 'session' | 'transaction' | 'statement', maxConnections: Number(form.get('max_connections')) })
