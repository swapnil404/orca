  ...
  return (
    <AuthPage eyebrow="Control plane access" title="Log in to Orca">
      <form method="post" className="space-y-4" onSubmit={handleSubmit}>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-[var(--radius-md)] bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-[#111205] transition duration-[var(--dur-fast)] ease-[var(--ease-premium)] hover:bg-[#fbff91] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 focus:ring-offset-[var(--card)] disabled:cursor-not-allowed disabled:opacity-60">
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[var(--bg)] px-5 py-12 text-[var(--text)]">
      ...
      <section className="relative w-full max-w-[380px] rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--card)] p-7 shadow-[0_28px_80px_rgba(0,0,0,0.45)] sm:p-8">
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--accent)]">{eyebrow}</p>
        <h1 className="mb-7 mt-2 text-2xl font-semibold tracking-[-0.03em]">{title}</h1>
        {children}
      </section>
    </main>
  )
...
function AuthField({ label, name, type, autoComplete }: AuthFieldProps) {
  return (
    <label className="block text-sm font-medium text-[var(--text-2)]">
      ...
      <input required name={name} type={type} autoComplete={autoComplete} className="w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--panel)] px-3.5 py-2.5 text-[var(--text)] outline-none transition duration-[var(--dur-fast)] placeholder:text-[var(--text-3)] focus:border-transparent focus:ring-2 focus:ring-[var(--accent)]" />
    </label>
  )
}
