...
Orca is a self-hosted Postgres orchestration and control platform. You run a small agent on your own server. The agent connects out to Orca's control plane over an encrypted tunnel, and from there you manage your Postgres infrastructure through a web UI: clusters, replicas, connection pooling, backups, extensions.

Orca never runs your database. Your data stays on your own infrastructure. Orca only stores what you want your infrastructure to look like, and pushes that down to the agent, which does the actual work through Docker.

## Why this exists

Managed Postgres works fine until you can't use it: compliance requirements, data residency, cost at scale, or just wanting your database on hardware you control. Orca is for that case. You get the control plane experience (canvas UI, one click to add a replica, visibility into topology and health) without handing your data to a third party.
...
3. You configure a Postgres cluster in the UI. That configuration is stored as desired state and pushed down to your agent over the same connection.
...
3. **Run that command on your server.** The agent starts, connects to Orca, and your host will show up as live in the UI within a few seconds. You don't need to open any inbound ports, the agent only connects out.
4. **Add a Postgres cluster** from the canvas. Set a name, version, and any config you want, then confirm.
...
From there, add replicas, connection pooling, or backups the same way: configure it in the UI, the agent reconciles it on your host.
...
- Removing a cluster, replica, or pool from the UI actually removes the corresponding container and data volume on your host. There's no separate "confirm deletion on the server" step, deletion in the UI is the source of truth.
...
├── server/   # control plane: API, WebSocket hub, desired state store
├── web/      # canvas UI
...
TBD, will be listed here once decided.

**Can I use my own Postgres image instead of the default?**
TBD.

**Is there a CLI?**
A CLI is planned. It isn't part of this rewrite yet, see the roadmap in Status above.

## Local development

Setup instructions will go here once there's something to set up.
